<script lang="ts">
    ...
    let { contentType, file, download_button }: Props = $props();
</script>

<div class="bg-ctp-crust rounded px-4 py-4">
    <div class="max-w-2xl mx-auto">
        {#if contentType.startsWith("image")}
            <img src={`/u/${file}`} alt="" class="max-h-[32em] mx-auto" />
        {:else if contentType.startsWith("video")}
            <video width="320" height="240" class="mx-auto" controls>
                <source src={`/u/${file}`} type={contentType} />
        ...
        {:else if contentType === "text/plain"}
            <iframe src={`/u/${file}`} class="w-full h-[32em] rounded"></iframe>
